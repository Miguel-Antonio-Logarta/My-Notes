...
def base_new_node_name(nodes):
    #Use names starting  with 'Z' for added nodes
    base_name = 'Z'
    for node in nodes:
        if node == base_name:
            base_name += 'Z'
...
as a series of edges, spearated by a semicolon (';') from a integer > 1.
...
graph "a,a b,c b,d b,e c,d c,e d,e" has an indpendent set of size 2,
...
c,f e,f" is a positive instance because {a, b, c} forms an indepdendent
...
def ConvertCliqueToHalfIndependentSet(clique_instance):
    ...
    ## Add the complement of clique_graph to the HalfIndependentSet graph 
    #
    ...
    #####  Midterm #2 count_new_nodes is the number of nodes to add, if any,
    #####  to the clique instance to make the HalfIndependentSet instance.
    #####  It is calculated based on the problem instance, not a constant
    #####  such as 1.
    #

    # We are given a positive instance of a graph that contains a clique.
    # Knowing this, we need to convert the clique into a graph that contains an independent set

    # clique_instance is positive if it has a clique size greater than the specified size k
    # an independent set contains nodes that are not connected to each other. So for example,
    # If we have an independent set of size 3 in a graph that has 6 nodes, that means
    # we have 3 nodes that are not connected to each other in the graph.

    # Since HlfIS_graph_edge_list is a complement of G, any nodes that weren't connected to each other in
    # G are not connected. All of these conected nodes, together is the size of our independent set

    # We need to get independent all unconnected nodes from I', we can get that from the original clique
    clique_nodes = set(nodes)
    connected_nodes = set()
    for edge in HlfIS_graph_edge_list:
        first_node, second_node = edge.split(',')
        connected_nodes.add(first_node)
        connected_nodes.add(second_node)

    # Get the unconnected nodes
    independent_nodes = clique_nodes.difference(connected_nodes)

    # Midterm #2
    # # Keep adding new nodes until the connected nodes in G' exceed more than half of the total number of nodes
    global count_new_nodes
    count_new_nodes = 0
    while count_new_nodes + len(independent_nodes) < (count_new_nodes + len(nodes)) / 2:
        count_new_nodes += 1
    
    base_name = base_new_node_name(nodes)

    # Create list of new nodes to add to the Clique instance to create
    #  the HalfIndepedentSet instance: Z0, Z1, Z2, ...
    new_nodes = [f'{base_name}{idx}' for idx in range(count_new_nodes)]

    iset_size = clique_size

    print("new_ndoes", new_nodes)
    print("set size", iset_size)
    ##### Midterm #2 The clique problem instance determines how new_nodes,
    ##### if there are any, are added to the HalfIndependentSet instance. This has
    ##### nothing to do with whether iset_size is odd or even, so
    ##### change the code below appropriately.
    #
    if iset_size*2 > len(independent_nodes): ##### Midterm #2 Change to correct boolean condition
        for new_node in new_nodes:
            # each new_node is connected only to itself -- that means they must be part
            # of an independent set
            # 
            HlfIS_graph_edge_list.append(f'{new_node},{new_node}') 

    elif iset_size < len(independent_nodes):  ##### Midterm #2 Change to correct boolean condition

        if len(new_nodes) == 1:
            ##### Midterm #2 — attaching a sinle new node to an arbitrary node won't work,
            ##### modify as needed.
            HlfIS_graph_edge_list.append(f'{new_nodes[0]},{nodes[0]}') 

        else:
            for idx,new_node_1 in enumerate(new_nodes[:-1]):
                for new_node_2 in new_nodes[idx+1:]:
                    # connect all the new nodes to each other.
                    HlfIS_graph_edge_list.append(f'{new_node_1},{new_node_2}') 

                ##### Midterm #2 Append other edges needed by HlfIS_graph_edge_list.
                #####
...
def vfyCliqueViaVfyHalfIndependentSet(clique_instance, S, H):
    ...
    new_nodes = []
    for idx in range( count_new_nodes ):
        new_nodes.append(f'Z{idx}')
    
    HlfIS_hint = f'{H} {" ".join(new_nodes)}'
    if VERBOSE:
        print(f'VERBOSE: vfyCliqueViaVfyHalfIndependentSet() - hint "{HlfIS_hint}"')
